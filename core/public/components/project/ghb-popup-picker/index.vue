 ...
 * @LastEditTime: 2019-09-02 16:56:49
 * @Description: 选择
 -->
<template>
    ...
    <div class="weui-cell vux-tap-active" :class="{'weui-cell_access': !disabled}" @click="onClick" v-show="showCell">
      ...
      <div class="vux-cell-primary vux-popup-picker-select-box">
        <div class="vux-popup-picker-select" :style="{textAlign: valueTextAlign}">
          <span class="vux-popup-picker-value vux-cell-value" :style="{color: value.length > 0 ? '#09122b' : '#999'}" v-if="!displayFormat && !showName && value.length">{{value | array2string}}</span>
          <span class="vux-popup-picker-value vux-cell-value" :style="{color: value.length > 0 ? '#09122b' : '#999'}" v-if="!displayFormat && showName && value.length">{{value | value2name(data)}}</span>
          <span class="vux-popup-picker-value vux-cell-value" :style="{color: value.length > 0 ? '#09122b' : '#999'}" v-if="displayFormat && value.length">{{ displayFormat(value, value2name(value, data)) }}</span>
          <span v-if="!value.length && placeholder" v-text="placeholder" class="vux-popup-picker-placeholder vux-cell-placeholder"></span>
        </div>
      </div>
      <div class="weui-cell__ft right_box">
      </div>
    </div>
...
<script>
import Picker from '../../base/v-picker'
import Cell from '../../base/v-cell'
import {Popup} from 'vux'
import PopupHeader from '../../base/v-popup-header'
import InlineDesc from '../../base/inline-desc'
import { Flexbox, FlexboxItem } from '../../base/v-flexbox'
import array2string from '../../vux-support/filters/array2String'
import value2name from '../../vux-support/filters/value2name'
import uuidMixin from '../../vux-support/libs/mixin_uuid'
import TransferDom from '../../vux-support/directives/transfer-dom'
