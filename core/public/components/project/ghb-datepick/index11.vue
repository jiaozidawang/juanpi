...
export default {
  ...
  created () {
    // let defaultDate = this.$tools.getCurrentMonthDate()
    // this.startDefault = defaultDate.monthStartDate;
    // this.endDefault = defaultDate.monthEndDate;
  },
  computed: {
    startDateInit () {
  ...
  methods: {
    ...
    showDatetime(val) {
      showDatePicker({title: '选择开始日期', dateFormat: 'yyyy-MM-dd'}).then(res => {
        console.log(res);
      })
    }
  }
