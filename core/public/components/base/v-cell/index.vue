<template>
  <div
    ...
    <div class="vux-cell-bd" :class="{'vux-cell-primary': primary === 'title' && valueAlign !== 'left'}">
      <p>
        <label class="vux-label" :style="labelStyles" :class="labelClass" v-if="title || hasTitleSlot">
          <slot name="title" :style="titleType">{{ title }}</slot>
        </label>
    ...
    <div class="weui-cell__ft" :class="valueClass" :style="valueStyle">
...
import cleanStyle from './utils/clean-style.js'
...
export default {
  ...
  methods: {
    ...
    //   !this.disabled && go(this.link, this.$router)
      !this.disabled && this.$emit('clickHandler',{title:this.title,value:this.value})
    }
  },
  data () {
    return {
