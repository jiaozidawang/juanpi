...
<script>
import { parentMixin } from '../../vux-support/mixins/multi-items.js';
...
export default {
  ...
  computed: {
    ...
    barStyle () {
      ...
      let mergeCommonStyle = Object.assign({}, commonStyle, this.costumBarStyle);
      if (!this.customBarWidth) {
        mergeCommonStyle.backgroundColor = this.barActiveColor || this.activeColor
      } else {
        mergeCommonStyle.backgroundColor = 'transparent' // when=prop:custom-bar-width
      }
      return mergeCommonStyle
    },
    barClass () {
      return {
