    ...
    <div class="ghb-bank-node-picker-popup" v-transfer-dom>
      <popup v-model="currentValue" height="80%" @on-hide="hidePopupHandler">
        <div class="popup-content">
          ...
          <div class="popup-content-content" v-if="!searchModel">
            <swiper
              height="100%"
              :threshold="200"
              :show-dot="false"
...
export default {
  ...
  methods: {
    ...
    provinceClickHandler(item) {
      ...
      const cityList = CITY.filter(el => {
        if (el.parent == item.code) {
          return el;
        }
      });
      console.log("城市列表", item, cityList);
    ...
    queryCity () {
      ...
      return this.$tools.rpc(apiList.queryCity, parms);
    },
    //查询网点
    searchBanknode(nodeName, code) {
      ...
      return this.$tools.rpc(apiList.queryBankNode, parms);
    },
